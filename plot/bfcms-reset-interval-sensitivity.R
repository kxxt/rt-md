...
df1 <- read_export_csv("reset-interval.csv", "w/ both allowlists")
...
p_fp <- ggplot(df_all, aes(x = R, y = FP, color = source)) +
  ...
  theme_academic +
  scale_y_continuous(
    breaks = sensitivity_y_ticks("bfcms_reset_interval_fp"),
    limits = sensitivity_y_limits("bfcms_reset_interval_fp", df_all$FP),
...
# plot(p_tp)
# plot(p_fp)
ggsave("plot/bfcms-reset-interval-fp.pdf", plot = p_fp, width = 7, height = 5, units = "in", dpi = 300)
ggsave("plot/bfcms-reset-interval-tp.pdf", plot = p_tp, width = 7, height = 5, units = "in", dpi = 300)
ggsave("plot/bfcms-reset-interval.pdf", plot = p, width = 7, height = 3, units = "in", dpi = 300)
# par(ask = FALSE)
